...
class TestPostReviewComment(unittest.TestCase):
    ...
    def test_post_review_comment_with_in_reply_to(self, mock_run_curl):
        mock_run_curl.return_value = (0, '{"done": true}', '')
        asyncio.run(post_review_comment('123', 'test.py', 1, 'test comment', labels={'Verified': 1}, gerrit_base_url='https://gerrit-review.googlesource.com', in_reply_to='abc123def'))
        expected_payload_str = '{"comments": {"test.py": [{"line": 1, "message": "test comment", "unresolved": true, "in_reply_to": "abc123def"}]}, "labels": {"Verified": 1}}'
        mock_run_curl.assert_called_with(
            [
                '-X',
                'POST',
