...
def _build_diff_header(file_path: str, diff_json: dict) -> str:
    """Build a summary header line from Gerrit's structured diff metadata.

    Returns a string like:
        diff path/to/file.py (MODIFIED)
        old: file.py (120 lines)
        new: file.py (125 lines)
    """
...
def _format_structured_diff(
    ...
    Format:
        <line_number_number>:  unchanged line
        <line_number_number>: +added line
                         : -deleted line
    """
    ...
    def _has_change(chunk: dict) -> bool:
    ...
    for i, chunk in enumerate(content):
        if "ab" in chunk:
            ...
            if skip_trimming:
                for text in ab:
                    lines.append(f"{line_number:>6}:  {text}")
                    line_number += 1
                continue
